...
const Cart = () => {
  const { cart, removeFromCart, cartTotal, setCartTotal } = useContext(AppContext);
  const [price, setPrice] = useState();

  const cartTotalFunc = () => {
    const total = cart.reduce((accumulator, cartItem) => {
      const product = Techproducts.find((p) => p.id === cartItem.item);
    ...
    <div>
      <div className="contact-parent">
        <ul className="contact-header">
          ...
          <li className="contact-text">Cart</li>
        </ul>
      </div>
      ...
      <span className="checkout-parent-grid">
        ...
        <div className="cart-total-parent">
          ...
          <div className="sub-total-div">
            <p>Subtotal</p>
            <p className="cart-price">${cartTotal}.00</p>
          </div>
          <hr />
          ...
          <div className="sub-total-div">
            <p>Total:</p>
            <p className="cart-price">${cartTotal + 10}.00</p>
          </div>
